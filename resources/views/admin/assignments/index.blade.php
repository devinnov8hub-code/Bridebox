    ...
    <main class="main">
        <header class="topbar">
            <div class="greeting">
                ...
                <p class="subtext">{{ __('Create and manage assignments based on lessons.') }}</p>
            </div>
            <div class="actions">
                <a class="btn primary" href="{{ route('admin.assignments.create') }}">{{ __('Add Assignment') }}</a>
                @php($exportQuery = request()->query())
                <a class="btn ghost" href="{{ route('admin.assignments.export', array_merge($exportQuery, ['format' => 'csv'])) }}">{{ __('Export CSV') }}</a>
                <a class="btn ghost" href="{{ route('admin.assignments.export', array_merge($exportQuery, ['format' => 'xlsx'])) }}">{{ __('Export Excel') }}</a>
                <a class="btn ghost" href="{{ route('admin.assignments.export', array_merge($exportQuery, ['format' => 'pdf'])) }}">{{ __('Export PDF') }}</a>
            ...
            <div class="panel-body">
                <div class="table-toolbar">
                    ...
                    <form class="search-form" method="get" action="{{ route('admin.assignments.index') }}">
                        ...
                        @if ($hasFilters)
                            <a class="btn ghost btn-small" href="{{ route('admin.assignments.index') }}">{{ __('Clear') }}</a>
                        @endif
                    </form>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="table-actions">
                                            <a class="btn ghost btn-small" href="{{ route('admin.assignments.submissions.index', $assignment) }}">{{ __('Submissions') }}</a>
                                            <a class="btn ghost btn-small" href="{{ route('admin.assignments.edit', $assignment) }}">{{ __('Edit') }}</a>
                                            <form method="post" action="{{ route('admin.assignments.delete', $assignment) }}" data-confirm="{{ __('Delete this assignment?') }}" style="display:inline-block;">
                                                @csrf
                                                @method('delete')
